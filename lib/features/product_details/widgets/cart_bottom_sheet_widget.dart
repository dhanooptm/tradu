...
class QuantityButton extends StatelessWidget {
  ...
  const QuantityButton({super.key,
    ...
    required this.stock,
    this.isCartWidget = false,required this.minimumOrderQuantity,required this.digitalProduct, this.withVariation = false, this.index = 0,
  });
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        if (!isIncrement && quantity! > 1) {
          if(quantity! > minimumOrderQuantity!) {
            if(withVariation){
              Provider.of<ProductDetailsController>(context, listen: false).setVariationQuantity(index, quantity! - 1);
            }else{
        ...
        }else if(!isIncrement && withVariation && quantity! > 0){
          Provider.of<ProductDetailsController>(context, listen: false).setVariationQuantity(index, quantity! - 1);
      ...
      child: Container(width: withVariation ? 25 : 40,height: withVariation ? 25 : 40,
        decoration: BoxDecoration(
        ...
        child: Icon(isIncrement ? Icons.add : Icons.remove,
          ...
          size: withVariation ? 20 : isCartWidget? 26 : 20,
        ),
      ),
    );
  }
