...
class OrderController with ChangeNotifier {
  ...
  Future<void> getOrderRequest(int offset,{bool isUpdate = true, int? limit = 10}) async {
    if(offset == 1){
      orderRequestModel = null;
    }
    ApiResponse apiResponse = await orderServiceInterface.getOrderRequestList(offset,limit);
    debugPrint('=============response==========>>>>${apiResponse.response?.data}');
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      if(offset == 1){
        orderRequestModel = OrderRequestModel.fromJson(apiResponse.response?.data);
      }else {
        orderRequestModel!.orders!.addAll(OrderRequestModel.fromJson(apiResponse.response?.data).orders!);
        orderRequestModel!.offset = OrderRequestModel.fromJson(apiResponse.response?.data).offset;
        orderRequestModel!.totalSize = OrderRequestModel.fromJson(apiResponse.response?.data).totalSize;
      }
    }else{
      ApiChecker.checkApi(apiResponse);
    }
