...
class ValidateCheck{
  ...
  static String? validateNumber(String? value) {
    const pattern = r'^[0-9]+$';
    final kNumberValid = RegExp(pattern);
    bool isValid = kNumberValid.hasMatch(value.toString());

    if (value!.isEmpty) {
      return '\u26A0 ${getTranslated('number_is_required', Get.context!)}';
    } else if (!isValid) {
      return '\u26A0 ${getTranslated("enter_valid_number", Get.context!)}';
    }
    return null;
  }
